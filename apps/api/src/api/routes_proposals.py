...
async def approve_proposal(
    ...
    def _log_task_exception(task: asyncio.Task):
        ...
        if not task.cancelled() and task.exception():
            logger.error("Execution task failed: %s", task.exception())
...
<body>
  <nav class="yc-navbar">
    <img class="yc-navbar-logo" src="https://auth.meshi.io/storage/v1/object/public/ascend-public/avatar/logo.png" alt="YesChef" />
    ...
    async with async_session() as db:
        ...
        try:
            if action_type == "gmail_draft":
                ...
                exec_result = await execute_calendar_action(
                    entity_id=entity_id,
                    title=title,
                    body=body,
                    session_id=session_id,
                )
            elif action_type == "design_prototype":
                exec_result = await execute_design_prototype(
                    title=title,
                    body=body,
                    session_id=session_id,
                )
            ...
            else:  # general_agent (default/fallback)
                exec_result = await execute_general_agent(
                    entity_id=entity_id,
                    title=title,
                    body=body,
                    recipient=recipient,
